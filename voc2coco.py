...
def convert(xml_dir, json_file):
    list_fp = glob.glob(xml_dir + '/*.xml')
    json_dict = {"images":[], "annotations": [], "categories": []}
    categories = PRE_DEFINE_CATEGORIES
    bnd_id = 1
    image_id = 1
    for line in list_fp:
        # line = line.strip() # remove spaces of head and tail
        ...
        for obj in get(root, 'object'):
            ...
            if category not in categories: # judge from key values
...
if __name__ == '__main__':
    ...
    # sys default converts input parameters into string type
    convert(sys.argv[1], sys.argv[2])
    # convert('F:\\UAV\\val\\Annotations\\', 'output.json')